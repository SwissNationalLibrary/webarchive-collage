...
const { vipsArrayJoin } = require('./lib/exec');
...
const matchSnapshotUrns = (files, snapshotsByUrn) => {
  ...
  files.forEach((filename) => {
    ...
    // new: bel-123456-nb-webarchive%2F20190911104851%2Fhttps%3A%2F%sample.ch.jpg
    // old: bel-123456-http%3A%2F%2Fpywb.ehelvetica.localhost%3A8088%2Fnb-webarchive%2F20190901163213%2Fhttps%3A%2F%2Fwww.sample.ch.jpg
    const fileUrn = filename.replace(/.*\/(.*?)(\!|\-http|\-nb|!)+.*/, '$1');
    const waybackUrl = decodeURIComponent(
      filename.replace(/.*(https?.*?).jpe?g$/, '$1')
    );
...
const createSuperRow = async (row, across, from, to, srcFun, dstFun) => {
  logger.debug(
    `creating super row from [${srcFun(from)} - ${srcFun(to)}] to ${dstFun(
      row
    )} across=${across}`
  );
  ...
  for (let i = from; i <= to; i++) {
    filenames.push(srcFun(i));
  }
  logger.debug(filenames);
  await vipsArrayJoin({
    images: filenames,
    across,
    targetFilename: dstFun(row),
  });
};
...
(async () => {
  ...
  await mkdirp(dstDir);
  await mkdirp(`${dstDir}/intermediate`);
  ...
  files = files.filter(
    (f) => f.match(/\.jpe?g/) && !f.match(/\.DS_Store|^undefined|~$/)
  );

  logger.debug(`matching ${files?.length} files with their metadata record`);
  matchSnapshotUrns(files, snapshotsByUrn);
  ...
  const columnsPerBlock = options.columnsPerBlock || 5;
  const rowsPerBlock = columnsPerBlock;

  // number of images per block
  const blockSize = options.blockSize ?? columnsPerBlock * rowsPerBlock;

  // number of blocks per row
  const blocksPerRow =
    options.blocksPerRow ??
    Math.ceil(Math.sqrt(urnKeys.length) / columnsPerBlock);

  // number of rows for a super block
  const superRowSize =
    options.superRowSize ?? Math.ceil(1000 / (blocksPerRow * columnsPerBlock));
  logger.trace(
    `using superRowSize=${superRowSize}, blockSize=${blockSize}, blocksPerRow=${blocksPerRow}, columnsPerBlock=${columnsPerBlock}`
  );

  let superRowStartBlock = 0;
  let superRowCount = 0;
  let blockCount = 0;
  let blockRowCount = 0;
  ...
  while (filesRemaining > 0) {
    ...
    block = Math.floor(offset / blockSize);
    const blockX = block % blocksPerRow;
    const blockY = Math.floor(block / blocksPerRow);
    ...
    const destFilename = `${dstDir}/intermediate/block-${block}.tif`;
    logger.trace(
      `creating block ${block} filename=${destFilename} working set size=${workingSet.length}`
    );

    if (!options['dry-run']) {
      // build parameters for working set (one block)
      const params = [];
      for (let i = 0; i < workingSet.length; i++) {
        const filename = workingSet[i];
        const urnId = getUrnIdFromFilename(filename);
        const col = i % columnsPerBlock;
        const row = Math.floor(i / +columnsPerBlock);
        ...
        const left = blockX * columnsPerBlock * +tileSizeX + col * +tileSizeX;
        const top = blockY * columnsPerBlock * +tileSizeY + row * +tileSizeY;
      ...
      const promise = sharp({
        create: {
          width: tileSizeX * columnsPerBlock,
          height: tileSizeY * Math.round(blockSize / columnsPerBlock),
          channels: 3,
        ...
        .toFile(destFilename)
        .then(() => {
          logger.debug(`-> written block ${destFilename}`);
        });

      // do some parallel processing
      console.log(promises.length, numProcesses);
      if (promises.length < numProcesses) {
        promises.push(promise);
      }

      logger.debug(
        `promises.length ${promises.length} ?= numProcesses ${numProcesses}`
      );

      if (promises.length === numProcesses || promises.length >= blocksPerRow) {
      ...
      blockCount++;
      if (blockCount % blocksPerRow === 0) {
        blockRowCount++;
        if (blockRowCount % superRowSize === 0) {
          // a new super row is reached
          await createSuperRow(
            superRowCount,
            blocksPerRow,
            superRowStartBlock,
            block,
            (block) => `${dstDir}/intermediate/block-${block}.tif`,
            (row) => `${dstDir}/row-${row}.tif`
          );
          superRowStartBlock = block;
          superRowCount++;
        }
      }
    } else {
      logger.debug('-> using dry montage - skip op');
    }
  ...
  // write out last super-row
  if (block > superRowStartBlock) {
    logger.debug(`flash last superrow start=${superRowStartBlock} to=${block}`);
    await createSuperRow(
      superRowCount,
      blocksPerRow,
      superRowStartBlock,
  ...
  const collageConfig = {
    ...
    numRows: superRowCount,
    itemsPerRow: blocksPerRow * columnsPerBlock,
